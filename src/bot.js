...
import {
  appendRowToTab,
  appendSubmission,
  deleteRowsByColumnValue,
  deleteSheetRow,
  findCitationByUsername,
...
async function handleCeDelete(interaction) {
  ...
  try {
    ...
    for (const tabName of tabs) {
      try {
        const deleted = await deleteRowsByColumnValue(tabName, offender, "B", 5);
        if (deleted > 0) {
          totalDeleted += deleted;
          perTab.push(`${tabName} (${deleted})`);
        }
      } catch (err) {
    ...
    if (!totalDeleted) {
      await interaction.editReply({
        content: `No Class-E sentence found for **${sanitizeForDisplay(offender)}** in any tab.`,
      });
      return;
    }
  ...
  try {
    ...
    await withMessageLock(interaction.message.id, async () => {
      ...
      await interaction.editReply({
        content:
          delta > 0
            ? `✅ ${jobLabel} point logged for **${officer}**. ${jobLabel} total: **${result.next}** · Monthly jobs: **${result.monthlyNext}**.`
            : `↩️ ${jobLabel} point removed for **${officer}**. ${jobLabel} total: **${result.next}** · Monthly jobs: **${result.monthlyNext}**.`,
      });
      ...
      try {
        ...
        if (logChannel) {
          ...
          await logChannel.send({
            content:
              delta > 0
                ? `💻 **${interaction.user.username}** authorized a **${jobLabel}** point for **${sanitizeForDisplay(officer)}** (${jobLabel}: ${result.next}, monthly: ${result.monthlyNext}). ${logLink}`
                : `↩️ **${interaction.user.username}** removed a **${jobLabel}** point from **${sanitizeForDisplay(officer)}** (${jobLabel}: ${result.next}, monthly: ${result.monthlyNext}). ${logLink}`,
          });
        }
      } catch (err) {
